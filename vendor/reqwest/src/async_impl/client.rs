...
struct Config {
    ...
    // Local patch: DSCP / IP_TOS marking (full TOS byte).
    tos: Option<u32>,
    // Local patch: per-connection TOS picker for weighted DSCP distributions.
...
impl ClientBuilder {
    ...
    pub fn build(self) -> crate::Result<Client> {
        ...
        connector_builder.set_tos(config.tos);
        connector_builder.set_tos_picker(config.tos_picker.clone());
        #[cfg(any(target_os = "android", target_os = "fuchsia", target_os = "linux"))]
    ...
    /// Local patch: set the IP type-of-service / IPv6 traffic-class byte
    /// (`IP_TOS` / `IPV6_TCLASS`) on all sockets this client opens. `tos` is the
    /// full 8-bit value (DSCP in the high 6 bits, ECN in the low 2). Used to
    /// mark download/upload throughput traffic for QoS classification.
    ///
    /// If `None`, the option is left at the system default.
    pub fn tos<T>(mut self, tos: T) -> ClientBuilder
    where
        T: Into<Option<u32>>,
    {
        self.config.tos = tos.into();
        self
    }

    /// Local patch: set a per-connection TOS picker. The closure runs once for
    /// each new connection this client opens and returns the full TOS byte to
    /// apply (or `None` for the system default). Used to spread connections
    /// across a weighted set of DSCP values. Takes precedence over [`tos`].
    #[allow(clippy::type_complexity)]
    pub fn tos_picker(
        mut self,
        picker: std::sync::Arc<dyn Fn() -> Option<u32> + Send + Sync>,
    ) -> ClientBuilder {
        self.config.tos_picker = Some(picker);
        self
    }
