...
where {
    ...
    // Local patch: forward the DSCP / IP_TOS marking to the underlying
    // hyper-util HttpConnector.
    pub(crate) fn set_tos(&mut self, tos: Option<u32>) {
        match &mut self.inner {
            #[cfg(feature = "default-tls")]
            Inner::DefaultTls(http, _tls) => http.set_tos(tos),
            #[cfg(feature = "__rustls")]
            Inner::RustlsTls { http, .. } => http.set_tos(tos),
            #[cfg(not(feature = "__tls"))]
            Inner::Http(http) => http.set_tos(tos),
        }
    }

    // Local patch: forward a per-connection TOS picker (weighted DSCP) to the
    // underlying hyper-util HttpConnector.
    #[allow(clippy::type_complexity)]
    pub(crate) fn set_tos_picker(
        &mut self,
        picker: Option<std::sync::Arc<dyn Fn() -> Option<u32> + Send + Sync>>,
    ) {
        match &mut self.inner {
            #[cfg(feature = "default-tls")]
            Inner::DefaultTls(http, _tls) => http.set_tos_picker(picker),
            #[cfg(feature = "__rustls")]
            Inner::RustlsTls { http, .. } => http.set_tos_picker(picker),
            #[cfg(not(feature = "__tls"))]
            Inner::Http(http) => http.set_tos_picker(picker),
        }
    }
