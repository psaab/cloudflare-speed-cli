...
struct Config {
    ...
    // Local patch: DSCP / IP_TOS marking (full TOS byte). See `set_tos`.
    tos: Option<u32>,
    // Local patch: per-connection TOS picker. When set, it is consulted for each
    // new connection and takes precedence over `tos`. Used for weighted DSCP
    // distributions. See `set_tos_picker`.
    #[allow(clippy::type_complexity)]
    tos_picker: Option<std::sync::Arc<dyn Fn() -> Option<u32> + Send + Sync>>,
...
impl<R> HttpConnector<R> {
    ...
    /// Local patch: set the IP type-of-service / traffic-class byte (`IP_TOS`
    /// for IPv4, `IPV6_TCLASS` for IPv6) on all sockets. `tos` is the full 8-bit
    /// value (DSCP in the high 6 bits, ECN in the low 2). If `None`, the option
    /// is left at the system default.
    #[inline]
    pub fn set_tos(&mut self, tos: Option<u32>) {
        self.config_mut().tos = tos;
    }

    /// Local patch: set a per-connection TOS picker. The closure is invoked once
    /// for every new connection and its return value (a full TOS byte, or `None`
    /// to leave the default) is applied to that connection's socket. Takes
    /// precedence over `set_tos`. Enables weighted DSCP distributions.
    #[inline]
    #[allow(clippy::type_complexity)]
    pub fn set_tos_picker(
        &mut self,
        picker: Option<std::sync::Arc<dyn Fn() -> Option<u32> + Send + Sync>>,
    ) {
        self.config_mut().tos_picker = picker;
    }
...
/// Local patch: apply the IP type-of-service / IPv6 traffic-class byte to a
/// socket. On unix this uses `setsockopt(IP_TOS)` / `setsockopt(IPV6_TCLASS)`
/// directly (socket2 0.5 has no IPv6 setter); elsewhere it falls back to
/// `socket2::set_tos` for IPv4 and is a no-op for IPv6.
#[cfg(unix)]
fn set_socket_tos(socket: &socket2::Socket, addr: &SocketAddr, tos: u32) -> io::Result<()> {
    ...
    // SAFETY: `value` is a valid `c_int` we own; the passed length matches its
    // size; the fd is borrowed from `socket` for the duration of the call.
    let ret = unsafe {
        libc::setsockopt(
            socket.as_raw_fd(),
            level,
...
fn set_socket_tos(socket: &socket2::Socket, addr: &SocketAddr, tos: u32) -> io::Result<()> {
    match addr {
        SocketAddr::V4(_) => socket.set_tos(tos),
        // socket2 0.5 exposes no IPv6 traffic-class setter; skip rather than fail.
        SocketAddr::V6(_) => Ok(()),
    }
}
...
fn connect(
    ...
    // Local patch: DSCP / IP_TOS marking. A per-connection picker (weighted
    // distribution) takes precedence over the fixed `tos`. Best-effort — a
    // failure here should not prevent the connection from being established.
    let tos = config
        .tos_picker
        .as_ref()
        .and_then(|pick| pick())
        .or(config.tos);
    if let Some(tos) = tos {
        if let Err(e) = set_socket_tos(&socket, addr, tos) {
